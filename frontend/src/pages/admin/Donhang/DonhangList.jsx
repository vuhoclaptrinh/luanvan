...
const DonhangList = () => {
  ...
  const donhangFiltered = Donhang.filter((dh) => {
    const matchSearch = dh.trang_thai
      ?.toLowerCase()
      .includes(searchText.toLowerCase());

    const matchKhachhang = selectedKhachhang
      ? dh.khach_hang_id === parseInt(selectedKhachhang)
      : true;

    const matchName = dh.ten_khach_hang
      ?.toLowerCase()
      .includes(searchName.toLowerCase());

    const matchTrangthai = selectedTrangthai
      ? dh.trang_thai === selectedTrangthai
      : true;

    return matchSearch && matchKhachhang && matchName && matchTrangthai;
  });
  ...
  const columns = [
    ...
    {
      field: "tong_tien_truoc_giam_fomat",
      headerName: " Tiền Gốc  ",
      width: 150,
    },
  ...
  return (
    <Box>
      ...
      <DonhangEdit
        ...
        donhangId={selectedDonhangId}
        onUpdate={fetchDonhang}
      />
      <DonhangView
        open={viewOpen}
