...
const DanhmucView = ({ open, onClose, DanhmucId }) => {
  ...
  useEffect(() => {
    if (open && DanhmucId) {
      setLoading(true);
      axios.get(`${API_BASE}danhmuc/${DanhmucId}`)
        .then((res) => {
          // Lấy data danh mục, kèm sản phẩm (sanphams)
          ...
          <Paper elevation={3} sx={{ p: 3, borderRadius: 2, backgroundColor: 'white' }}>
            <Grid container spacing={3}>
              <Grid item xs={12} md={7}>
                ...
                {/* Hiển thị danh sách sản phẩm thuộc danh mục */}
                {danhmuc.sanphams && danhmuc.sanphams.length > 0 && (
                  <Box mt={3}>
                    ...
                    {danhmuc.sanphams.map((sp) => (
                      <Box key={sp.id} display="flex" gap={2} mb={0.5}>
                        <Typography sx={{ width: 100, fontWeight: 500 }}>ID: {sp.id}</Typography>
                        <Typography>Tên: {sp.ten_san_pham}</Typography>
                      </Box>
                    ))}
                  </Box>
                )}
