...
const ProductGrid = ({
 ...
 // const [daDanhGiaMap, setDaDanhGiaMap] = useState({});
  ...
  // Kiểm tra đã mua sản phẩm
      ...
      try {
        const res = await fetch(`http://127.0.0.1:8000/api/danhgia/kiemtra-da-mua/${khachHangId}/${productId}`);
  ...
  const fetchRatings = async () => {
    ...
    await Promise.all(
      filteredProducts.map(async (product) => {
        try {
          const res = await fetch(`http://127.0.0.1:8000/api/danhgia/trungbinh/${product.id}`);
          const data = await res.json();
...
const fetchReviewed = async () => {
  ...
  await Promise.all(
    filteredProducts.map(async (product) => {
      try {
        const res = await fetch(`http://127.0.0.1:8000/api/danhgia/da-danh-gia/${khachHangId}/${product.id}`);
        const data = await res.json();
...
useEffect(() => {
  if (filteredProducts.length > 0 && khachHangId) {
    fetchReviewed();
  }
              ...
              <div className={`d-flex ${viewMode === "list" ? "flex-row" : "flex-column"}`}>
                ...
                <Card.Body className="d-flex flex-column">
                    ...
                    {ratings[product.id] !== undefined && (
                      <div className="text-warning mb-1">
                        {Array.from({ length: 5 }, (_, i) => (
                          <i
                            key={i}
                            className={`bi ${i < ratings[product.id] ? "bi-star-fill" : "bi-star"}`}
                          ></i>
                        ))}
                        <small className="ms-1 text-muted">({ratings[product.id]} sao)</small>
                      </div>
                    )}
                  ...
                  {daMuaMap[product.id] === false && (
                    <div className="text-muted small">Chỉ khách đã mua mới được đánh giá</div>
                  )}
                  ...
                  {daMuaMap[product.id] === true && (
                    <form
                      ...
                      onSubmit={async (e) => {
                        ...
                        const noi_dung = reviewContent[product.id]?.trim();
                        if (!noi_dung) {
                        ...
                        try {
                          const res = await fetch("http://127.0.0.1:8000/api/danhgia", {
                            method: "POST",
                          ...
                          // ✅ Reset và ẩn form
                          setReviewedMap((prev) => ({ ...prev, [product.id]: true }));
                          setReviewContent((prev) => ({ ...prev, [product.id]: "" }));
                          setSelectedStars((prev) => ({ ...prev, [product.id]: 5 }));
                          fetchRatings();
                        } catch (err) {
                    ...
                    >
                      <div className="d-flex align-items-center gap-1 mb-2">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <i
                            key={star}
                            className={`bi ${
                              (selectedStars[product.id] || 5) >= star ? "bi-star-fill" : "bi-star"
                            } text-warning`}
                            style={{ cursor: "pointer" }}
                            onClick={() =>
                              setSelectedStars((prev) => ({ ...prev, [product.id]: star }))
                            }
                          />
                        ))}
                      </div>
                      <textarea
                        ...
                        onChange={(e) =>
                          setReviewContent((prev) => ({ ...prev, [product.id]: e.target.value }))
                        }
                      />
