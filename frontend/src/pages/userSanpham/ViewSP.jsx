...
const ViewSP = () => {
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        const uniqueCategories = [
          ...new Set(productsData.map((p) => p.danh_muc_ten).filter(Boolean)),
        ];
        const uniqueBrands = [
          ...new Set(productsData.map((p) => p.thuong_hieu).filter(Boolean)),
        ];
  ...
  const handleShowMore = () => {
    setVisibleCount((prevCount) =>
      Math.min(prevCount + 8, filteredProducts.length)
    );
  };
