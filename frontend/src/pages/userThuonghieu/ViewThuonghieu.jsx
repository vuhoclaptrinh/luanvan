    ...
    const fetchBrandProducts = async () => {
      ...
      try {
        ...
        const uniqueCategories = [
          ...new Set(brandProducts.map((p) => p.danh_muc_ten).filter(Boolean)),
        ];
        const uniqueBrands = [
          ...new Set(brandProducts.map((p) => p.thuong_hieu).filter(Boolean)),
        ];

        // B5: Tính giá cao nhất
        const prices = brandProducts
          .map(
            (p) => Number.parseFloat(p.gia_format?.replace(/[^\d]/g, "")) || 0
          )
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="keywords"
          content={`${decodedBrand}, nước hoa, perfume, ${categories.join(
            ", "
          )}`}
        />
      </Helmet>
        ...
        <Container className="mb-4">
          ...
          >
            <div className="position-relative z-2">
              ...
              {brandInfo.mo_ta && (
                <p className="lead mb-3 opacity-90">{brandInfo.mo_ta}</p>
              )}
              ...
              {products.length === 0 ? (
                <div className="text-center py-5 bg-white rounded shadow-sm">
                  ...
                  <p className="text-muted">
                    Thương hiệu "{decodedBrand}" hiện không có sản phẩm nào.
                  </p>
                  <Button
                    variant="primary"
                    onClick={() => window.history.back()}
